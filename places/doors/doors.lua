...
if typeof(isfolder) == "function" and typeof(isfile) == "function" then
    if isfolder("mspaint\\doors\\addons") then
        ...
        for _, file in pairs(listfiles("mspaint\\doors\\addons")) do
            if file:sub(#file - 3) == ".lua" or file:sub(#file - 4) == ".luau" then
                local success, errorMessage = pcall(function()
                    ...
                    if typeof(addon.Name) ~= "string" or typeof(addon.Elements) ~= "table" then
                        warn("Addon '" .. string.gsub(file, "mspaint/doors/addons/", "") .. "' didn't load: Invalid Name/Elements.")
                        return 
                    end
                    ...
                    if not Tabs["Addons"] then
                        Tabs.Addons = Window:AddTab("Addons [BETA]")
                    ...
                    local AddonGroupbox = LastGroupbox == "Right" and Tabs.Addons:AddLeftGroupbox(addon.Title) or Tabs.Addons:AddRightGroupbox(addon.Title);
                ...
                if not success then
                    warn("[mspaint] Failed to load addon '" .. string.gsub(file, "mspaint/doors/addons/", "") .. "':", errorMessage)
                end
            end
        end
    end
